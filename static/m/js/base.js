                ...
                success : function(data){
                    if(data.status==200){
                                ...
                                for(var i=0; i<listArray.length; i++){
                                    ...
                                    if(p==1){
                                        var imgSrc = list['img'] + '_200x200.jpg';
                                    }else{
                                        var imgSrc = data.goodsBgimg +'" data-original="' + list['img'] + '_200x200.jpg';
                                    }
                                    ...
                                    content += '<img src="'+ imgSrc + '" class="cxx-image lazy"></a></div>';
                                    ...
                                    content += '<div class="am-cf am-margin-bottom-xs" style="line-height:2.3rem"><span class="cxx-text-pink">￥<em>' + parseFloat(list['discount_price']).toFixed(1) + '</em></span>';
                                    content += '<span class="am-text-xs cxx-color-gray am-margin-left-xs am-padding-top-xs" style="text-decoration:line-through">￥' + parseFloat(list['price']).toFixed(1) +'</span></div>';
                                    ...
                                    content += '<span class="am-text-sm">￥</span><span class="am-text-xl">' + ((list['coupon_price'] >= 10) ? parseFloat(list['coupon_price']).toFixed(0) : parseFloat(list['coupon_price']).toFixed(1))  + '</span></div><span class="am-text-xs" style="color:rgba(0,0,0,0.5);">优惠券</span><br><a href="javascript:void(0);" data-id="'+list['goods_id'] + '" class="cxx-goods-buy am-text-sm cxx-text-pink am-radius am-block am-padding-xs am-margin-top-sm">领券购买</a><div class="am-round bottom-found am-inline-block am-text-center"></div></div></div></div></div></li>';    
                                }
